...
// файла манифеста целят в один и тот же dest внутри одного профиля. syncFiles() в этом случае
// молча пишет оба (побеждает последний в списке), а checkFiles() вечно видит дрейф по
// проигравшей записи. Гвард включает все with-группы разом, чтобы поймать дубликат, который
